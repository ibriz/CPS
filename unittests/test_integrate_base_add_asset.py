...
class BalancedTestBaseAddAsset(BalancedTestUtils):
    ...
    def _deploy_all(self):
        ...
        core_contracts = ["daofund", "dex", "dividends", "reserve", "rewards", "loans"]
        ...
        loans_deploy_tx = self.deploy_tx(
            from_=self.btest_wallet,
            to=self.contracts.get(core_contracts[0], SCORE_INSTALL_ADDRESS),
            content=os.path.abspath(os.path.join(self.CORE_CONTRACTS_PATH, core_contracts[0])),
            params={"_governance": self.contracts[governance]}
        )
        self.contracts[governance] = loans_deploy_tx[SCORE_ADDRESS]
        pprint(loans_deploy_tx)
        ...
        # band_deploy_tx = self.deploy_tx(
        #     from_=self.staking_wallet,
        #     to=self.contracts.get(bandOracle, SCORE_INSTALL_ADDRESS),
        #     content=os.path.abspath(os.path.join(self.CORE_CONTRACTS_PATH, bandOracle)),
        #     params={"_ref": self.contracts['oracle'], "_staking": self.contracts['staking']}
        # )
        # self.contracts[bandOracle] = band_deploy_tx[SCORE_ADDRESS]
