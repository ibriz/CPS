...
const EmailConfirmationModal = ({ user, resendVerificationEmailRequest, setModalShow, address, ...props }) => {
  ...
  return (
    <Modal
      ...
      style={{ color: '#262626' }}
    >
      <Modal.Header closeButton style={confirmationShow ? { backgroundColor: '#DDDDDD' } : {}}>
        <Modal.Title id="contained-modal-title-vcenter" style={{ color: "#262626" }}>
          {verified ? 'Email Verified' : 'Email Verfication Required'}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body className={styles.modalBody} style={confirmationShow ? { backgroundColor: '#DDDDDD' } : {}}>
        {
          verified ?
