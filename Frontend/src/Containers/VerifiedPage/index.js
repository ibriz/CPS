...
const VerifiedPage = ({ hasAddress, history }) => {
    ...
    let [unsubscribeConfirmationShow, setUnsubscribeConfirmationShow] = React.useState(false);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%' }}>
            <Container style={{ color: '#262626' }}>
                ...
                <div className={styles.confirmationMessage}>Thank you for verifying your email address. You will now receive email notification if you have enabled them.<span style={{ fontWeight: '600' }}>{walletAddress}</span></div>
                <div className={styles.optOutButtonContainer}>
                    <Button variant="primary" className={styles.optOutButton} onClick={() => history.push('./dashboard')}>Continue to Dashboard</Button>
                </div>
            </Container>
            <Footer console />
