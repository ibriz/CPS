...
import {fetchVoteResultBudgetChangeSuccess, fetchVoteResultBudgetChangeFailure} from '../../Reducers/progressReportSlice';
...
function* fetchVoteResultBudgetChangeRequestWorker({payload}) {
  try {
    ...
    yield put(fetchVoteResultBudgetChangeSuccess(
      {
        response,
      }
    ));
