...
const proposalSlice = createSlice({
    ...
    reducers: {
                ...
                proposal => (
                    {
                        ...
                        budget: IconConverter.toBigNumber(proposal[PARAMS.totalBudget]).dividedBy(10**18),
                        ...
                        projectDuration: IconConverter.toBigNumber(proposal[PARAMS.projectDuration]),
        ...
        fetchProjectAmountsSuccess(state, action) {
            ...
            for(const proposalStatus of proposalStatusMapping) {
                state.projectAmounts[proposalStatus.name] = {
                    amount: parseFloat(IconConverter.toBigNumber(action.payload[proposalStatus.status]?._total_amount ?? 0).dividedBy(10**18)),
        ...
        fetchSponsorMessageSuccess(state, action) {
            state.sponsorMessage[action.payload.response.ipfsKey] = action.payload.response.message;
            return;
        },  
...
export const { submitProposalRequest, submitProposalSuccess, submitProposalFailure,
    ...
    setBackendTriggerData, setSponsorRequestProposalTitle, setSponsorRequestProposal} = proposalSlice.actions;
